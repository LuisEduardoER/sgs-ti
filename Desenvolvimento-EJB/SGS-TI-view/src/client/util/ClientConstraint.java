package client.util;

public class ClientConstraint {
	// Constantes
	public static final String CONTEUDO_INICIAL = "CONTEUDO_INICIAL";
	
	public static final String CONTEUDO_FINANCEIRO = "CONTEUDO_FINANCEIRO";
	public static final String CONTEUDO_UTILIDADES = "CONTEUDO_UTILIDADES";
	
	public static final String SIDE_MENU_CLIENTE = "SIDE_MENU_CLIENTE";
	public static final String SIDE_MENU_CHAMADO = "SIDE_MENU_CHAMADO";
	public static final String SIDE_MENU_AGENDA = "SIDE_MENU_AGENDA";
	
	
	public static final String LISTAR_CHAMADOS = "LISTAR_CHAMADOS";
	public static final String EDITAR_CHAMADOS = "EDITAR_CHAMADOS";
	public static final String CRIAR_CHAMADOS = "CRIAR_CHAMADOS";
	public static final String LISTAR_AGENDA = "LISTAR_AGENDA";
	
	public static final String STATUS_SISTEMA = "STATUS_SISTEMA";
	public static final String SHOW_USERS	 = "SHOW_USERS";
	
	
	
}
